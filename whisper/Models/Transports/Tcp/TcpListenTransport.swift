...
final class TcpListenTransport: SubscribeTransport {
    ...
    func startDiscovery() -> TransportDiscovery {
        ...
        discoveryTimer = Timer.scheduledTimer(withTimeInterval: 2, repeats: false) { _ in
            if self.whisperers.isEmpty {
                let whisperer = Remote(id: "Tcpr-1", name: "Seku")
    ...
    private func readChunks() -> [TimedChunk] {
        do {
            ...
            let url = folderURL.appendingPathComponent("TcpTimedChunks.json")
            if let data = try? Data(contentsOf: url) {
                return try JSONDecoder().decode([TimedChunk].self, from: data)
            }
            guard let url = Bundle.main.url(forResource: "TcpTimedChunks", withExtension: "json") else {
                logger.error("Missing TcpTimedChunks.json from bundle")
                return []
            }
            ...
            logger.error("Failed to read TcpTimedChunks: \(err)")
            return []
        }
    }
